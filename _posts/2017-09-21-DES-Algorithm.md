...
* S-Box中的每一行是整数0-15的一个置换
* S-Box不是它输入变量的线性或仿射函数
* S-Box的输入端每改变1位至少要引起输出端改变2位
* S(X) 和S(X + 001100) 至少有2位不同
* 对6位二进制串X = x1x2x3x4x5x6，S(X) != S(X + {11x5x600})
* S-Box的输入端保持任1位不变，则其它输入位的变化输出数字中0 和 1 的总数近于相等。
